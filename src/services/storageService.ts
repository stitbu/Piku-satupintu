...
export const StorageService = {
  ...
  getUsers: (): User[] => {
    ...
    // If local storage exists, return it (to support CRUD changes)
    if (stored) return JSON.parse(stored);

    // If empty, INITIALIZE from constants
  ...
  deleteUser: (userId: string) => {
      const users = StorageService.getUsers();
      // Prevent deleting self or super admin if needed, though UI should handle this
  ...
  login: (identifier: string, credential?: string): User | null => {
      ...
      if (mockUser) {
          // Find the current version of this user in LS to get updated profile
          const lsUser = allUsers.find(u => u.id === mockUser.id);
          return lsUser || mockUser;
      }
